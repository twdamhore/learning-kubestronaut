...
**Explanation:** Both /etc/kubernetes/admin.conf (the admin kubeconfig generated by kubeadm) and ~/.kube/config (the default user kubeconfig location) can contain cluster connection information. The admin.conf is typically copied to ~/.kube/config for kubectl to use by default, but kubectl can use either via the KUBECONFIG environment variable.
